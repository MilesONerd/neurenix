...
impl OneAPIBackend {
    ...
    fn initialize(&mut self) -> Result<()> {
        ...
        if !Self::is_oneapi_available() {
            return Err(PhynexusError::BackendNotAvailable("oneAPI is not available on this system".to_string()));
        }
        
        self.platform_ids = self.get_platform_ids()?;
        
        if self.platform_ids.is_empty() {
            return Err(PhynexusError::BackendNotAvailable("No oneAPI platforms found".to_string()));
        }
        ...
        if self.device_ids.is_empty() {
            return Err(PhynexusError::BackendNotAvailable("No oneAPI devices found".to_string()));
        }
...
impl super::Backend for OneAPIBackend {
