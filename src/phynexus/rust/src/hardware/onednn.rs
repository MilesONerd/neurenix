...
impl OneDNNBackend {
    ...
    fn initialize(&mut self) -> Result<()> {
        ...
        if !Self::is_onednn_available() {
            return Err(PhynexusError::BackendNotAvailable("oneDNN is not available on this system".to_string()));
        }
        
        let cpu_engine = self.create_cpu_engine()?;
        self.engines.push(cpu_engine);
        
        if let Ok(gpu_engine) = self.create_gpu_engine() {
            self.engines.push(gpu_engine);
        }
        
        if self.engines.is_empty() {
            return Err(PhynexusError::BackendNotAvailable("No oneDNN engines could be created".to_string()));
        }
    ...
    fn create_gpu_engine(&self) -> Result<*mut std::ffi::c_void> {
        Ok(null_mut())
    }
...
impl super::Backend for OneDNNBackend {
